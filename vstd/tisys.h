...
{
public :
    typedef TVDynamicSystemImpBase2< TSysTrait , IVSysDynamic< TRs ... > > IMP_BASE ;
...
////////////////////////////////////////////////////////////
template< typename TR , typename TIMP > class TVRSimpPipeLine ;

template< typename TID , typename TIMP > 
class TVRSimpPipeLine< IVRLTN< TID > , TIMP > : public TVRSimpPipeLine< typename IVRLTN< TID >::HUB , TIMP > 
{} ;
...
private :
    TVSvcConverter< TIMP , TOUT , TINPUTs ... > m_sysPipe ;
} ;

template< typename TIMP , typename TPRT > class TVRSimpManager ;

template< typename TIMP , typename TID , typename TOUT , typename ... TI > 
class TVRSimpManager< TIMP , IVRUniqHub< TID , TOUT , TI ... > > 
{
public :
    TVRSimpManager( TVRSimpPipeLine< IVRLTN< TID > , TIMP > & pl , IVSlot< TI > & ... locs  , IVInputPort< TOUT > & inp )
    { 
        pl.Create( m_node ) ;
        VLNK( inp , m_node ) ;
        VLNK( m_node , locs ... ) ;
...
{
    typedef TVRSimpPipeLine< TRROOT , TIMP >  PIPELINE ;

    template< typename TR > struct TRTrait
    {
        typedef TVRSimpManager< TIMP , typename TRROOT::HUB > mngr_type ;
    };
} ;
...
template< typename T , T > struct TVConverterTraitBase ;

template< typename TIMP , typename R , typename TOUTPUT , typename ... TINPUTs , R (TIMP::*mf)( TOUTPUT & , const TINPUTs & ... ) >
struct TVConverterTraitBase< R ( TIMP::* )( TOUTPUT & , const TINPUTs & ... ) , mf > 
{
...
using TVConverterTrait = TVConverterTraitBase< decltype( &TIMP::Build ) , &TIMP::Build > ;
...
template< typename TCNVR > struct TVDeduceConverterWrapper ;

template< typename TOUTPUT , typename ... TINPUTs > struct TVDeduceConverterWrapper< IVConverter< TOUTPUT , TINPUTs ... > >
{
...
#define VDDP_NODE_IMP( name , trait )   typename trait::server_type m_sysSvc##name ;\
    typedef typename TVDeduceConverterWrapper< typename trait::node_type >::type name ;\
    ...
    VD_DECLAREEC_IMP( m_sysSvc##name , VD_EXPAND( __COUNTER__ ) )
...
#define VDDP_NOD( name , type )  VDDP_NODE_IMP( name , TVConverterTrait< type > )
