...
int main()
{
	...
	DcmItem dt2(DcmTagKey(0x0009, 0x11A2));
	DcmItem dt3(DcmTagKey(0x0009, 0x11A3));
	//DcmSequenceOfItems  sq(PRIVATE_ELEMENT);
	//
	//sq.append(&dt1);
	//sq.append(&dt2);
	//sq.append(&dt3);
	...
	registerPrivateTags();
	DcmSequenceOfItems  sq(PRV_PrivateElement1);
	sq.insert(&dt1);
	sq.insert(&dt2);
	//DcmDataset dd;
	//OFCondition f = dd.putAndInsertTagKey(PRV_PrivateCreator, DcmTagKey(0x0009, 0x11A1));
	//f = dd.setVR(EVR_SQ);
	//dd.insert(&sq);
	//dd.insert(&sq);
	//dd.insert(&sq);
